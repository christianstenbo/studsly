...
export const strings = deepFreeze({
  ...
  register: {
    ...
    // Built, but not switched on for this account. Distinct from "Coming
    // soon" on purpose: one is a missing feature, the other is a flag, and
    // conflating them is what made three working flows look unbuilt.
    notEnabled: 'Not switched on for you',
    ...
    // Flow 4 — standalone instructions & boxes (FF_COMPONENTS).
    part: {
      pageTitle: 'Register a loose part — Studsly',
      back: '← Register',
      title: 'Register a loose part',
      subtitle: 'A part you own that is not locked inside a built set. It joins your free pool and can be allocated to any set missing that piece.',
      partLabel: 'Part number',
      partPlaceholder: 'e.g. 3004',
      partHint: 'The catalogue part number — printed in instructions and on BrickLink.',
      partFound: (name: string) => `Catalogue: ${name}`,
      partNotFound: 'Not in the catalogue. You can still register it — the number is stored as you typed it.',
      colourLabel: 'Colour',
      colourPlaceholder: 'Choose a colour',
      colourSearch: 'Search colours…',
      colourHint: 'Required. Every slot a part can fill has a colour, so a part without one can never be matched to what a set is missing.',
      nameLabel: 'Name (optional)',
      namePlaceholder: 'Filled in from the catalogue when we recognise the number',
      quantityLabel: 'How many',
      quantityHint: 'One entry with a count — not one entry per brick.',
      locationLabel: 'Location (optional)',
      locationPlaceholder: 'e.g. Loftstue',
      subLocationLabel: 'Box or drawer (optional)',
      subLocationPlaceholder: 'e.g. Box no 2',
      missingPart: 'Enter a part number.',
      missingColour: 'Choose a colour — it is what lets Studsly match this part to a set that needs it.',
      saveFailed: 'Could not register that part. Please try again.',
      submit: 'Add to my free pool',
      submitting: 'Registering…',
      success: 'Added to your free pool',
      successId: (id: string) => `Registered as ${id}. It is now available to allocate.`,
      successSub: 'It is now available to allocate.',
      addAnother: 'Add another',
      viewPool: 'View my pool',
    },
    component: {
      pageTitle: 'Register instructions or a box — Studsly',
      back: '← Register',
      title: 'Instructions or a box',
      subtitle: 'Register a spare manual or original box on its own. Link it to a set later from that set’s Contents.',
      typeLabel: 'What is it?',
      typeInstruction: '📘 Instructions',
      typeBox: '📦 Original box',
      setLabel: 'Which set is it for?',
      setPlaceholder: 'Set number, e.g. 10185',
      setHint: 'The set number lets Studsly show this in that set’s Contents.',
      setFound: (name: string) => `Matched: ${name}`,
      setNotFound: 'No catalog match — you can still register it.',
      nameLabel: 'Name (optional)',
      namePlaceholder: 'e.g. Winter Village Bakery',
      locationLabel: 'Location (optional)',
      locationPlaceholder: 'Storage, shelf B…',
      gradeLabel: 'Condition (optional)',
      submit: 'Register',
      submitting: 'Registering…',
      missingSet: 'Enter the set number this belongs to.',
      saveFailed: 'Could not register. Please try again.',
      successInstruction: 'Instructions registered.',
      successBox: 'Original box registered.',
      addAnother: 'Register another',
